...
Затем владелица открывает `nano /opt/oracle/.env` и вписывает ID приложения VK ID вместо `PASTE_VK_CLIENT_ID`. **Пустых переменных быть не должно.** Если приложения VK ID ещё нет, временно `VK_CLIENT_ID=0`: сайт запустится, не будет работать только вход.
